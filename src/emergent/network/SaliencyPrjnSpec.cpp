...
void SaliencyPrjnSpec::Init_Weights_Prjn(Projection* prjn, ConState_cpp* cg, Network* net,
                                         int thr_no) {
  ...
  for(suc.y = 0; suc.y < fltsz; suc.y++) {
    for(suc.x = 0; suc.x < fltsz; suc.x++) {
      ...
      if(wt > 0) {
        for(int sui=0;sui<su_gp->size;sui++) {
          if(sui == feat_no)
            cg->Cn(su_idx++,ConState_cpp::WT,net->net_state) = wt; // target feature
          else
            cg->Cn(su_idx++,ConState_cpp::WT,net->net_state) = 0.0f; // everyone else
        }
      }
      else {
        for(int sui=0;sui<su_gp->size;sui++) {
          if(sui != feat_no && sui >= fg_st && sui < fg_ed)
            cg->Cn(su_idx++,ConState_cpp::WT,net->net_state) = -surr_mult * wt;
          else
            cg->Cn(su_idx++,ConState_cpp::WT,net->net_state) = 0.0f; // not in our group or is guy itself
        }
      }
    }
  }
