...
#include "TiledDivGpRFPrjnSpec.h"
#include <Network>
#include <int_Array>
#include <taMath_float>
#include <taMisc>

TA_BASEFUNS_CTORS_DEFN(TiledDivGpRFPrjnSpec);

void TiledDivGpRFPrjnSpec::Initialize() {
  send_gp_size = 4;
  send_gp_skip = 2;
  send_gp_start = 0;
...
void TiledDivGpRFPrjnSpec::UpdateAfterEdit_impl() {
...
void TiledDivGpRFPrjnSpec::Connect_impl(Projection* prjn, bool make_cons) {
  ...
  taVector2i ruc;
  int rgpidx = 0;
  for(ruc.y = 0; ruc.y < ru_geo.y; ruc.y++) {   // for each recv group in y
    for(ruc.x = 0; ruc.x < ru_geo.x; ruc.x++, rgpidx++) {   // in x
      taVector2i su_st;
      ...
      taVector2i suc;
      taVector2i suc_wrp;
      for(suc.y = su_st.y; suc.y < su_st.y + send_gp_size.y; suc.y++) {    // for each group we will send to
        for(suc.x = su_st.x; suc.x < su_st.x + send_gp_size.x; suc.x++) {   // likewise in x
          suc_wrp = suc;
...
bool TiledDivGpRFPrjnSpec::TrgRecvFmSend(int send_x, int send_y) {
...
bool TiledDivGpRFPrjnSpec::TrgSendFmRecv(int recv_x, int recv_y) {
