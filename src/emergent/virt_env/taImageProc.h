...
class E_API taImageProc : public taNBase {
  // #STEM_BASE ##CAT_Image ##STATIC_COMPLETION basic image processing operations; images are either rgb = 3 dimensional matrix with 3rd dim = 3 or greyscale (2 dim matrix)
  INHERITED(taNBase)
public:
  ...
  static bool   CropImage_float(float_Matrix& crop_img, const float_Matrix& orig_img, 
                                ...
                                bool random_origin = false, int origin_x = -1, int origin_y = -1);
