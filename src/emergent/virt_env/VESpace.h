...
class E_API VESpace : public taNBase {
  // #STEM_BASE ##CAT_VirtEnv ##EXT_vespc #EXPAND_DEF_6 a virtual environment that represents a sub-space of objects, containing static elements only -- helps optimize the collision detection process to group proximal items into sub spaces
INHERITED(taNBase)
public:
  enum SpaceType {
  ...
  // #CAT_ODE #BUTTON #DYN1 apply (multiply) rotation around given axis to current rotation values -- IMPORTANT: axis values cannot all be 0 -- it will automatically normalize though (in radians: 180deg = 3.1415, 90deg = 1.5708, 45deg = .7854)
  virtual void  RotateEuler(float euler_x, float euler_y, float euler_z);
  // #CAT_ODE #BUTTON #DYN1 apply (multiply) rotation using given Euler angles to current rotation values (in radians: 180deg = 3.1415, 90deg = 1.5708, 45deg = .7854)
