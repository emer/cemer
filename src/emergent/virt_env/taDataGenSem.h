...
public:
  ...
  static bool	SemVecGen_Flip
    (bool toward, DataTable* dest, DataTable* dest_tmp, Variant dest_name_col,
     ...
     DataTable* bit_freqs,  bool use_k_thr = true, float softmax_thr = 0.0f,
     float softmax_gain = 0.4f, float p_flip_items = 0.1f, float freq_weight = 0.1f);
  // #CAT_SemVecGen #MENU_BUTTON #MENU_ON_SemVecGen generate semantic vector patterns in destination column of dest datatable (which must exist and is used for starting state -- duplicate prior to calling if you want to save it) by flipping individual bits in the patterns.  Bits are chosen according to a softmax function weighted by the sum of distance * other rep's bits -- bits are flipped in on/off pairs to ensure that kwta constraint is preserved.   toward = flip to increase similarity of closest patterns, else flip weights to decrease similarity of most dissimilar patterns.  use_k_thr = only focus on the top/bottom k closest items (as represented in dist_kmat table -- top for toward, bottom for away).  softmax_gain = multiplier on softmax -- 0 = uniform probability, higher values = use weighted probabilities more.  p_flip_items = proportion of items that get their bits flipped -- this should be as small as possible (such that one flip occurs) toward the end of the process, but can be higher earlier on.  freq_weight and bit_freqs drive constraint toward keeping bits relatively uniform in frequency across patterns -- penalizes turning on bits that are already high frequency, and turning off ones that are already low frequency

  static bool	SemVecGen_FlipStats
    (DataTable* dest, Variant dest_name_col, Variant dest_vec_col,
     DataTable* dist_mat, DataTable* lrn_stats, DataTable* dest_dist, 
     float softmax_gain = 0.4f, float p_flip_items = .01f,
