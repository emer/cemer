...
public:
  ...
  float         ti_scale_mult;  // #CONDSHOW_ON_on&&burst&&ti_rescale multiplier on the deep5b relative netinput scaling factor, for how much to subtract from the ti_ctxt inputs for the ti_rescale function -- ti_ctxt *= (1 - ti_scale_mult * d5b_rel_scale) -- this overall rescaling factor cannot go below zero -- a value of 1.0 should exactly compensate for the extra d5b input, but larger values may be needed depending on relative scales and level of excitation communicated by the deep5b activations -- monitor the avg_act_diff variable on the LeabraLayer to see these effects
