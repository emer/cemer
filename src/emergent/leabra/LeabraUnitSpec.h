...
class E_API ShortPlastSpec : public SpecMemberBase {
  // ##INLINE ##NO_TOKENS ##CAT_Leabra short-term plasticity specifications -- algorithm CYCLES: uses standard equations summarized in Hennig, 2013 (eq 6) to capture both facilitation and depression dynamics as a function of presynaptic firing -- models interactions between number of vesicles available to release, and probability of release, and a time-varying recovery rate -- rate code uses generated spike var to drive this; algorithm TRIAL_BINARY: units continously above thresh for n_trials will depress to 0 activity and individually recovery at rec_prob to full strength on trial by trial basis 
INHERITED(SpecMemberBase)
public:
    enum STPAlgorithm {
        CYCLES,
  ...
  STPAlgorithm algorithm;        // #CONDSHOW_ON_on
  float         f_r_ratio;      // #CONDSHOW_ON_on&&algorithm:STANDARD #DEF_0.01:3 ratio of facilitating (t_fac) to depression recovery (t_rec) time constants -- influences overall nature of response balance (ratio = 1 is balanced, > 1 is facilitating, < 1 is depressing).  Wang et al 2006 found: ~2.5 for strongly facilitating PFC neurons (E1), ~0.02 for strongly depressing PFC and visual cortex (E2), and ~1.0 for balanced PFC (E3)
  float         rec_tau;        // #CONDSHOW_ON_on&&algorithm:STANDARD #DEF_100:1000 #MIN_1 [200 std] time constant in cycles, which should be milliseconds typically (roughly, how long it takes for value to change significantly -- 1.4x the half-life) for the constant form of the recovery of number of available vesicles to release at each action potential -- one factor influencing how strong and long-lasting depression is: nr += (1-nr)/rec_tau.  Wang et al 2006 found: ~200ms for strongly depressing in visual cortex and facilitating PFC (E1), 600ms for depressing PFC (E2), and between 200-600 for balanced (E3)
  float         p0;             // #CONDSHOW_ON_on&&algorithm:STANDARD #DEF_0.1:0.4 [0.2 std] baseline probability of release -- lower values around .1 produce more strongly facilitating dynamics, while .4 makes depression dominant -- interacts with f_r_ratio time constants as well.  Tuning advice: keeping all other params at their default values, and focusing on depressing dynamics, this value relative to p0_norm = 0.2 can give different degrees of depression: 0.2 = strong depression, 0.15 = weaker, and 0.1 = very weak depression dynamics
  float         p0_norm;        // #CONDSHOW_ON_on&&algorithm:STANDARD #DEF_0.1:1 [0.2 std] baseline probability of release to use for normalizing the overall net synaptic transmitter release (syn_tr) -- for depressing synapses, this should be = p0, but for facilitating, it make sense to normalize at a somewhat higher level, so that the syn_tr starts out lower and rises to a max -- it maxes out at 1.0 so you don't want to lose dynamic range
  float         kre_tau;        // #CONDSHOW_ON_on&&algorithm:STANDARD #DEF_100 time constant in cycles, which should be milliseconds typically (roughly, how long it takes for value to change significantly -- 1.4x the half-life) on dynamic enhancement of time constant of recovery due to activation -- recovery time constant increases as a function of activity, helping to linearize response (reduce level of depression) at higher frequencies -- supported by multiple sources of biological data (Hennig, 2013)
  float         kre;            // #CONDSHOW_ON_on&&algorithm:STANDARD #DEF_0.002;0 how much the dynamic enhancement of recovery time constant increases for each action potential -- determines how strong this dynamic component is -- set to 0 to turn off this extra adaptation
  float         thresh;      // #CONDSHOW_ON_on&&algorithm:TRIAL_BINARY #DEF_0.5 the levels of activation in q3 over which a unit is subject to accomodation
  int           n_trials;    // #CONDSHOW_ON_on&&algorithm:TRIAL_BINARY no. of continious trials above threshold after which accomodation occurs
  float         rec_prob;         // #CONDSHOW_ON_on&&algorithm:TRIAL_BINARY #MIN_0 #MAX_1 accomodated units recover independently with recovery probability on a trial by trial basis 
  float         fac_tau;          // #CONDSHOW_ON_on&&algorithm:STANDARD #READ_ONLY #SHOW auto computed from f_r_ratio and rec_tau: time constant in cycles, which should be milliseconds typically (roughly, how long it takes for value to change significantly -- 1.4x the half-life) for the dynamics of facilitation of release probability: pr += (p0 - pr) / fac_tau. Wang et al 2006 found: 6ms for visual cortex, 10-20ms strongly depressing PFC (E2), ~500ms for strongly facilitating (E1), and between 200-600 for balanced (E3)
  float         fac;            // #CONDSHOW_ON_on&&algorithm:STANDARD #DEF_0.2:0.5 #MIN_0 strength of facilitation effect -- how much each action potential facilitates the probability of release toward a maximum of one: pr += fac (1-pr) -- typically right around 0.3 in Wang et al, 2006

  float         rec_dt;                // #CONDSHOW_ON_on&&algorithm:STANDARD #READ_ONLY #EXPERT rate constant for recovery = 1 / rec_tau
  float         fac_dt;         // #CONDSHOW_ON_on&&algorithm:STANDARD #READ_ONLY #EXPERT rate constant for facilitation =  1 / fac_tau
  float         kre_dt;         // #CONDSHOW_ON_on&&algorithm:STANDARD #READ_ONLY #EXPERT rate constant for recovery enhancement = 1 / kre_tau
  float         oneo_p0_norm;   // #CONDSHOW_ON_on&&algorithm:STANDARD #READ_ONLY #EXPERT 1 / p0_norm
