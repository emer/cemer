...
public:
  ...
  bool          zero_norm;  // #CONDSHOW_ON_on normalize the total deep5b activations per layer by turning the zero activations into negative values proportional to the mean activity within the layer
  bool          ffcc_off;   // #CONDSHOW_ON_on&&burst turn off the feedforward corticocortical pathway that parallels the d5b pathway, whenever the deep5b pathway is otherwise engaged -- only relevant with burst mode active -- this is important for avoiding a "main effect" dynamic where the extra deep5b activation causes units to just be overall more active (monitor avg_act_diff to see) -- biologically, the layer 4 neurons may provide this modulation of feedforward input pathways, so that either, but not both, pathways contribute (d5b pathway dominates when it is present, but ffcc fills in when it is not)
  bool          ti_off;     // #AKA_ti_rescale #CONDSHOW_ON_on&&burst turn off the TI context inputs to compensate for the extra deep5b net input when it is present -- only relevant with burst mode active -- this is important for avoiding a "main effect" dynamic where the extra deep5b activation causes units to just be overall more active (monitor avg_act_diff to see) -- biologically, the TI context is thought to be communicated via thalamocortical projections, which are then taken over by deep5b driven activations when those are present
