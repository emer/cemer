...
public:
  ...
  bool          rescale;        // #CONDSHOW_ON_on&&burst automatically rescale the relative net input scales per quarter to include deep5b projections when they are active, and to exclude them when they are not active -- only relevant with burst mode active -- this is important for avoiding a "main effect" dynamic where the extra deep5b activation causes units to just be overall more active (monitor avg_act_diff to see) -- see scale_mult parameter too -- an additional or alternative mechanism to use for this is ti_rescale -- it is preferred for units that get TI context inputs, as it does not require reinitializing the net inputs, which incurs a small performance penalty
  float         scale_mult;    // #CONDSHOW_ON_on&&burst&&rescale multiplier on the deep5b relative netinput scaling factor that is used in rescaling the other projections, for the rescale option -- setting this to a value larger than 1 will have the effect of downscaling the other projections even more than is needed to compensate for the additional deep5b inputs -- this can be useful given that the deep5b activations may be stronger overall than other inputs, so more compensation is required to prevent main effects -- monitor the avg_act_diff variable on the LeabraLayer to see these effects
  bool          ti_rescale;    // #CONDSHOW_ON_on&&burst rescale the TI context inputs to compensate for the extra deep5b net input when it is present, and also exclude deep5b from relative scaling entirely -- only relevant with burst mode active -- this is important for avoiding a "main effect" dynamic where the extra deep5b activation causes units to just be overall more active (monitor avg_act_diff to see) -- see ti_scale_mult parameter too -- this is preferred vs. rescale for units that get TI context inputs, as it does not require reinitializing the net inputs, which incurs a small performance penalty -- biologically, a portion of the TI context is thought to come from thalamocortical projections, which are then taken over by deep5b driven activations when those are present
  float         ti_scale_mult;  // #CONDSHOW_ON_on&&burst&&ti_rescale multiplier on the deep5b relative netinput scaling factor, for how much to subtract from the ti_ctxt inputs for the ti_rescale function -- ti_ctxt *= (1 - ti_scale_mult * d5b_rel_scale) -- this overall rescaling factor cannot go below zero -- a value of 1.0 should exactly compensate for the extra d5b input, but larger values may be needed depending on relative scales and level of excitation communicated by the deep5b activations -- the d5b_rel_scale can be around .01 so the useful range of this parameter can be up to around 100 or so -- monitor the avg_act_diff variable on the LeabraLayer to see these effects
