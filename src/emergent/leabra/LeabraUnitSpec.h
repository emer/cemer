...
public:
  ...
  float         min;            // #DEF_0.1 #MIN_0 miniumum avg_l value -- when unit activation is less than act_thr, then we decrease avg_l in a soft-bounded way toward this min value
  float         tau;            // #DEF_10 #MIN_1 time constant for updating avg_l -- rate of approaching soft exponential bound to max / min
  ...
  float         lrn_min;        // #DEF_0.005 #MIN_0 miniumum avg_l_lrn -- if avg_l is at its minimum value, then avg_l_lrn will be at this minimum value -- neurons that are not overly active may not need to increase the contrast of their weights as much
  ...
  float         err_min;        // #DEF_0.001:0.03 #CONDSHOW_ON_err_mod minimum layer.cos_diff_avg_lrn value (for non-zero cases) -- ensures a minimum amount of self-organizing learning 
