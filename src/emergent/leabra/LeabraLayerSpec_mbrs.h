...
public:
  ...
  float         fb_tau;         // #CONDSHOW_ON_on #MIN_0 #DEF_1.4;3;5 time constant in cycles, which should be milliseconds typically (roughly, how long it takes for value to change significantly -- 1.4x the half-life) for integrating feedback inhibitory values -- prevents oscillations that otherwise occur -- the fast default of 1.4 should be used for most cases but sometimes a slower value (3 or higher) can be more robust, especially when inhibition is strong or inputs are more rapidly changing
...
private:
  ...
  void	Defaults_init() {
    gi = 1.8f;    ff = 1.0f;    fb = 1.0f;    fb_tau = 1.4f;
...
INHERITED(SpecMemberBase)
public:
  float         pre_hog_thr;       // #MIN_0 #MAX_1 #DEF_0.2;0.25;0.15 threshold on unit avg_act (long time-averaged activation), above which (and below hog_thr) the unit is considered to be a 'pre-hog' that is likely on its way toward dominating the representational space -- drives pre_hog_pct in layer stats
...
private:
  void	Initialize() {   Defaults_init(); }
  void	Defaults_init() {
    pre_hog_thr = 0.2f;        hog_thr = 0.3f;    dead_thr = 0.01f;
  }
};
