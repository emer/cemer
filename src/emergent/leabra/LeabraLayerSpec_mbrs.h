...
#pragma maketa_file_is_target LeabraLayerSpec_cpp
...
public:
  ...
  bool          use_ext_act;  // #DEF_false WARNING: NOT SUPPORTED YET if true, then use the activation level computed from the external inputs to this layer (avg of targ or ext unit vars) -- this will only be applied to layers with INPUT or TARGET / OUTPUT layer types, and falls back on the targ_init value if external inputs are not available or have a zero average -- implies fixed behavior
