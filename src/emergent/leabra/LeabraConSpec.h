...
public:
  ...
  bool          nrm_l;          // use normalized long-term average activations (avg_l_nrm) instead of raw values -- this is more robust to layer-level differences in overall activation levels, etc
  float         l_mult;         // #CONDSHOW_ON_nrm_l multiplier on the normalized long-term average, to get it back into appropriate scale for learning
