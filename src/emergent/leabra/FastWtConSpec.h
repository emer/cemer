...
// todo: this code needs to be integrated with the stable weights -- is non-functional
// right now and only here for future reference

eTypeDef_Of(FastWtConSpec);

class E_API FastWtConSpec : public LeabraConSpec {
  // fast weight connection: standard wt learns fast, but decays toward slow weight value -- WARNING THIS CODE IS NONFUNCTIONAL AT THE MOMENT
INHERITED(LeabraConSpec)
public:
  enum FastWtConVars {
    SDWT = SWT+1,               // slow delta-weight
  };
