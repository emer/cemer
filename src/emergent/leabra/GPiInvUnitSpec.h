...
public:
  float         net_gain;        // #DEF_1 extra netinput gain factor to compensate for reduction in netinput from subtracting away nogo -- this is IN ADDITION to adding the nogo factor as an extra gain: net = (net_gain + nogo) * (go_in - nogo * nogo_in)
  float         nogo;            // #MIN_0 #DEF_1;0.1 how much to weight NoGo inputs relative to Go inputs (which have an implied weight of 1.0)
  float         gate_thr;        // #DEF_0.1 threshold applied to activation to drive gating -- when any unit activation gets above this threshold, it sends the activation through sending projections to the thal field in Matrix units, otherwise it sends a 0
  ...
  float         min_thal;        // minimum thal value to send to thal on layers we project to -- range between gate_thr and 1.0 is mapped into min_thal:1.0 range -- e.g., setting to 1.0 will effectively produce binary gating outputs -- set to 0 (or < gate_thr) to retain raw gate_thr to 1.0 range
