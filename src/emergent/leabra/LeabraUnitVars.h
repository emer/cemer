...
public:
  ...
  float      avg_m_eff;      // #CAT_Learning effective medium time-scale activation average that is actually used for the minus phase for learning in XCAL algorithms -- can include a contribution from ml trace average as well
  float      avg_ml;         // #CAT_Learning medium-long time-scale activation average -- integrates over avg_m values at the trial level, and adds a longer-term trace factor to the minus phase for learning in XCAL algorithms
  ...
  float      deep_ctxt;       // #NO_SAVE #VIEW_HOT #CAT_Activation temporally-delayed local lateral integration of deep_raw signals to provide context for temporal integration (TI) learning -- added into net input of superficial neurons -- copied from deep_ctxt_net at start of new alpha trial
  float      deep_raw_norm;  // #NO_SAVE #CAT_Activation the value of deep_raw that goes into the deep_norm computation for this unit -- determined by the deep_norm.raw_val setting on the unit spec -- automatically max-normalized to 1 per layer, and thresholded by deep_norm.raw_thr
  float      deep_norm;       // #NO_SAVE #VIEW_HOT #CAT_Activation normalized deep layer activation, reflecting normalization from deep_ctxt local integration of deep_raw signals -- this is the net attentional filter that is applied to the superficial layers, and is communicated in top-down deep-to-deep projections via layer 5/6 corticocortical neurons and communicated to layer 4 and thalamus by layer 6 corticothalamic projections, where it modulates net input into superficial neurons
  float      deep_mod;   // #NO_SAVE #VIEW_HOT #CAT_Activation current modulating value of deep_norm to apply to superficial neurons -- this is copied from deep_norm at the start of a new trial -- deep_norm itself is updated dynamically during the plus phase but does not take effect until it hits this variable
  float      deep_norm_net;   // #NO_SAVE #CAT_Activation net input from deep_norm activation of other areas, reflecting e.g., the layer 6 cortico-cortical projections that are the source of the top-down deep-to-deep pathway in the cortex, or the layer 6 corticothalamic projections into the thalamus
  float      deep_raw_net;    // #NO_SAVE #CAT_Activation net input from the deep_raw activation of other areas, in terms of the feedforward corticothalamic pathway from deep5b IB neurons to thalamic relay neurons in other areas
  float      deep_ctxt_net;   // #NO_SAVE #CAT_Activation local lateral integration of deep_raw signals, to drive normalization of the overall attentional filtering signals as reflected in deep_norm -- also provides temporal context for temporal integration (TI) learning in deep_ctxt
  float      thal;           // #NO_SAVE #VIEW_HOT #CAT_Activation thalamic activation value, driven by a ThalSendUnitSpec or GpiInvUnitSpec -- used by deep params in LeabraUnitSpec and MatrixConSpecs, and possibly other specs, to respond to thalamic inputs
