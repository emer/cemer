...
public:
  ...
  float      act_g;         // #CAT_Activation records the activation state at the time of the gating decision -- is either act_eq or act_nd depending on act_misc.rec_nd setting
  ...
  float      avg_m;          // #VIEW_HOT #CAT_Learning medium time-scale activation average -- integrates over avg_s values, and represents the minus phase for learning in XCAL algorithms
  float      avg_l;          // #VIEW_HOT #CAT_Learning long time-scale average of medium-time scale (trial level) activation, used for the BCM-style floating threshold in XCAL
  ...
  float      margin;         // #CAT_Activation relative status of this unit for the overall activation state / attractor, used for favoring units on the edges or margins of the attractor: -1 = below the low threshold -- solidly OFF, -0.5 = above the low threshold but below the midway threshold for the marginal units, +0.5 = above the midway threshold but still in the margin, +1 = above the high threshold and solidly within the main attractor state
