...
bool DRNUnitSpec::CheckConfig_Layer(Layer* ly, bool quiet) {
...
void DRNUnitSpec::Compute_Se(LeabraLayer* lay, LeabraNetwork* net) {
  float pospv = 0.0f;
  int   pospv_n  = 0;
  float negpv = 0.0f;
  int   negpv_n  = 0;
...
void DRNUnitSpec::Send_Se(LeabraLayer* lay, LeabraNetwork* net) {
  FOREACH_ELEM_IN_GROUP(LeabraUnit, u, lay->units) {
    if(u->lesioned()) continue;
...
void DRNUnitSpec::Compute_CycleStats(LeabraLayer* lay, LeabraNetwork* net, int thread_no) {
  Compute_Se(lay, net);
  Send_Se(lay, net);
  inherited::Compute_CycleStats(lay, net, thread_no);
}

void DRNUnitSpec::Init_Weights_Layer(LeabraLayer* lay, LeabraNetwork* net) {
