...
public:
  ...
  bool		vm_mod;		// #CONDSHOW_ON_gelin if using gelin, modulate the g_e excitatory conductance (i.e., net input) term used in gelin computation as function of ratio of v_m / v_m_asymp (max of 1) where v_m is current membrane potential, and v_m_asymp is asymptotic membrane potential for current inputs -- this avoids problem where all activations immediately reach their kwta-designated levels because kwta is computed based on g_e (netin) -- need v_m to integrate so that units reach this kwta level gradually, with strongest getting there more quickly, etc
