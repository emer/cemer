...
public:
  ...
  bool		veto;		// #CONDSHOW_ON_gelin use veto model of inhibition and resulting activation function -- activity is linear function of the ge net input (truly ge-lin), with time-integration of act instead of vm_mod -- vm_mod_max is max netinput value (for now)
...
public:
  ...
  float		thr_min;	// #DEF_0.01 #CONDSHOW_ON_l_sq maximum possible threshold value -- only in effect for l_sq case -- prevents excessive ltp for lower threshold values
