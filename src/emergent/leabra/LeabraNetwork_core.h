...
//{
  ...
  STATE_CLASS(LeabraNetDeep)    deep;     // #CONDEDIT_ON_main_obj flags for what elements of deep computation need to be performed -- typically auto-detected based on the network configuration in Trial_Init_Specs
...
#ifdef __MAKETA__
int   cycle;          // #NO_SAVE #GUI_READ_ONLY #SHOW #CAT_Counter #VIEW #VIEW_WIDTH_4 cycle counter: number of iterations of activation updating (settling) on the current alpha-cycle (100 msec / 10 Hz) trial -- this counts time sequentially through the entire trial, typically from 0 to 99 cycles (equivalent to the old ct_cycle, not cycle within a phase as cycle used to be prior to version 7.1.0) -- updated in CycleRun, not updated by program!
#endif

int             quarter;        // #NO_SAVE #GUI_READ_ONLY #SHOW #CAT_Counter #VIEW #VIEW_WIDTH_4 current gamma-frequency (25 msec / 40 Hz) quarter of alpha-cycle (100 msec / 10 Hz) trial being processed -- updated by State
Phase           phase;          // #NO_SAVE #GUI_READ_ONLY #SHOW #CAT_Counter #VIEW #VIEW_WIDTH_8 type of settling phase (MINUS or PLUS) -- minus is first 3 quarters, plus is last quarter -- updated by State
int   tot_cycle;      // #NO_SAVE #GUI_READ_ONLY #SHOW #CAT_Counter #VIEW #VIEW_WIDTH_8 total cycle count -- this increments from last Init_Weights and just keeps going up (unless otherwise reset) -- used for tracking things like spiking times continuously across time -- updated by State

float           rt_cycles;      // #NO_SAVE #GUI_READ_ONLY #SHOW #CAT_Statistic #VIEW #VIEW_WIDTH_4 cycles to generate an output response (reaction time = RT) -- typically lstats.trg_max_act_crit is used as an activation criterion over the trg_max_act value recorded from target output layer(s) to determine this -- updated by State
...
float           ext_rew;        // #NO_SAVE #GUI_READ_ONLY #SHOW #CAT_Statistic #VIEW #VIEW_WIDTH_8 external reward value (on this trial) -- only computed if ExtRewLayerSpec or similar exists in network
