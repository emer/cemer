...
//{
  ...
  // #IGNORE LeabraAvgMaxRaw data for layers, by thread
  ...
  INLINE STATE_CLASS(LeabraAvgMax)* ThrLayAvgMax(int thr_no, int lay_idx, AvgMaxVars var) 
  { return (STATE_CLASS(LeabraAvgMax)*)
      (thrs_lay_avg_max_vals[thr_no] + (n_layers_built * var + lay_idx) *
