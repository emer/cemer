...
#include "taiWidgetGroupElMenu.h"
...
taiWidgetGroupElMenu::taiWidgetGroupElMenu(taiWidgetActions::RepType rt, int ft, taList_impl* lst, TypeDef* typ_, IWidgetHost* host_, 
  taiWidget* par, QWidget* gui_parent_, int flags_)
: taiWidgetListElMenu(rt,ft,lst, typ_, host_,par, gui_parent_, flags_)
{
}

taiWidgetGroupElMenu::taiWidgetGroupElMenu(taiWidgetMenu* existing_menu, taList_impl* gp, TypeDef* tp, IWidgetHost* host_, taiWidget* par,
      QWidget* gui_parent_, int flags_) // uses flags: flgNullOk, flgNoGroup (aka flgNoList), flgNoInGroup, flgEditOk
: taiWidgetListElMenu(existing_menu, gp, tp, host_,par, gui_parent_, flags_)
{
}

void taiWidgetGroupElMenu::ChooseGp() {
...
void taiWidgetGroupElMenu::GetMenu_impl(taList_impl* cur_lst, taiWidgetActions* menu, iMenuAction* actn) {
  ...
  if (cur_gp->size >= taMisc::max_menu) {
    iAction* mnel = menu->AddItem
      ("<Over max, select...>", taiWidgetMenu::normal,
       iAction::action, this, SLOT(Choose()), cur_gp );
    over_max = true;
...
//TODO Qt4            taiWidgetMenu* sub_menu = menu->AddSubMenu(nm, (void*)tmp_grp);
      ...
      if (!added_sub) {
	menu->AddItem((char*)nm, taiWidgetMenu::use_default, actn, tmp);
      }
    }
  }
  if (cur_gp->gp.size >= taMisc::max_menu) {
    iAction* mnel = menu->AddItem
      ("gp.<Over max, select...>", taiWidgetMenu::normal,
        iAction::action, this, SLOT(ChooseGp()), &(cur_gp->gp) );
    over_max = true;
    if (actn != NULL) {		// also set callback action!
      mnel->connect(actn);
    }
