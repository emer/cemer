...
#include "taiWidgetDimEdit.h"
#include <iDimEdit>
#include <MatrixGeom>


taiWidgetDimEdit::taiWidgetDimEdit(TypeDef* typ_, IWidgetHost* host_, taiWidget* par, QWidget* gui_parent_, int flags_)
:taiWidget(typ_, host_, par, gui_parent_, flags_)
{
  Initialize(gui_parent_);
}

void taiWidgetDimEdit::Initialize(QWidget* gui_parent_) {
...
void taiWidgetDimEdit::GetImage(const MatrixGeom* arr) {
...
void taiWidgetDimEdit::GetValue(MatrixGeom* arr) const {
  iDimEdit* de = rep(); // cache
  arr->SetDims(de->dims());
