...
#include "taiWidgetEnumTypeDefChooser.h"
...
taiWidgetEnumTypeDefChooser::taiWidgetEnumTypeDefChooser(TypeDef* typ_, IWidgetHost* host,
...
bool taiWidgetEnumTypeDefChooser::AddType_Enum(TypeDef* typ_, TypeDef* par_typ) {
...
void taiWidgetEnumTypeDefChooser::btnHelp_clicked() {
  TypeDef* typ = td();
  if (!typ) return;
  iHelpBrowser::StatLoadEnum(typ);
}

void taiWidgetEnumTypeDefChooser::BuildChooser(iDialogItemChooser* ic, int view) {
  inherited::BuildChooser(ic, view);
  if (!targ_typ) {
    taMisc::Error("taiWidgetEnumTypeDefChooser::BuildChooser: targ_type needed");
    return;
  }
  switch (view) {
  case 0:
...
int taiWidgetEnumTypeDefChooser::BuildChooser_0(iDialogItemChooser* ic, TypeDef* top_typ,
...
int taiWidgetEnumTypeDefChooser::columnCount(int view) const {
  switch (view) {
  case 0: return 3;
...
const String taiWidgetEnumTypeDefChooser::headerText(int index, int view) const {
  switch (view) {
  case 0: switch (index) {
    case 0: return "Type";
    case 1: return "Enum";
