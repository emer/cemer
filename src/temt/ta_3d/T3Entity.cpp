...
void T3Entity::removeAllChildren() {
  ...
  for(int i = ol.count()-1; i >= 0; i--) {
    ...
    if(nd) {                    // only delete entities, not components!!
      ...
      // nd->deleteLater(); // no deleting -- causes crashes -- threads still have these guys hanging around -- 
      // not sure what to do about the orphans though?  surely we'll be getting leaks?
    }
  }
}
