...
public: // public API
  ...
  // #CAT_CtrlPanel #MENU #MENU_ON_ControlPanel #MENU_SEP_BEFORE #CONFIRM #DYN1 set all members to be STABLE -- you can then selectively mark a subset as ACTIVE -- in general don't want too many active fields
  virtual void       AllLocked();
  // #CAT_CtrlPanel #MENU #MENU_ON_ControlPanel #CONFIRM #DYN1 set all members to be LOCKED (not subject to editing of any sort) -- you can then selectively mark a subset as STABLE (editable but not recorded) or ACTIVE -- use this to lock down a very stable set of parameters and prevent further editing
  ...
  virtual String ActiveMembersToString(bool use_search_vals = false);
  // #CAT_CtrlPanel return a String with a list of space-separated name=value pairs for the active members in this control panel (if use_search_vals, records current search value) -- any ControlPanel's that are linked within this one are recursively processed as well (but its members are not scoped in any way, so for this to be useful, names in general must be unique across the different control panels)

  virtual void  SaveNameValueMembers_impl(ParamSet* param_set, String_Array& name_vals);
  // #IGNORE save name_vals name=value settings (generated by ActiveMembersToString, typically via ClusterRun, can be based on a prior set of param configs) to given param_set, creating members by copying from this, and recursively calling any linked param sets -- remove items from param_vals (each element is a name=value pair) when they have been processed -- outer loop will deal with the remainders
  virtual void  SaveNameValueMembers(ParamSet* param_set, const String& name_vals_str);
  // #CAT_CtrlPanel save name_vals_str name=value settings (generated by ActiveMembersToString, typically via ClusterRun, can be based on a prior set of param configs) to given param_set, creating members by copying from this, and recursively calling any linked param sets
  ...
  virtual void  SetMembersFromArgs(bool active_only = false, bool follow_control_panel_links = true);
  // #CAT_Args set single-valued members (optionally only those marked as ACTIVE for ControlPanel and ParamSet) from command-line arguments -- this is called automatically by the ControlPanelsFmArgs Program element -- optionally follows any members that are pointers to other control panels, to add them as well (default is true)
