...
public:
  ...
  Patch_Group           patches; // #NO_DIFF patches (collections of edit changes) that can be used for updating projects based on changes made to other projects -- can be generated by DiffCompare function, and loaded from files or web
  ...
  virtual void          SvnDiffPatch();
  // #BUTTON #CAT_File generate a new patch record based on differences between current project and prior svn checkin -- checks out the current version of the project as a tmp project file, loads it, runs a project-wide diff compare staged by each member of the project, and saves any marked changed to a new patch record in project
