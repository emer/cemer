...
public:
  ...
  bool          use_search_algo;     // #NO_DIFF use search algorithm to explore across parameters -- if false, then just the current values will be used -- must also set cur_search_algo
  ParamSearchAlgoRef cur_search_algo; // #NO_DIFF #CONDEDIT_ON_use_search_algo The current search algorithm in use -- if not set, then jobs will just use current parameters, for manual param searching -- see also use_search_algo

  bool          set_proj_name;  // #NO_DIFF set the project name to use -- overrides the default which is to use the actual name of the project -- this can be useful for running multiple variants of the same project with different local file names, all under a common cluster-run project name, so they can all share the same results etc
  ...
  bool          is_updating;    // #READ_ONLY #SHOW #NO_SAVE #METHBOX_LABEL  shows if the background SVN update is currently ongoing.
  ...
  String        notes;          // #NO_DIFF Notes about this run, used as a checkin comment and visible in job lists -- very good idea to be specific here.  Use %varname to automatically add current variable value
  String        label;          // #NO_DIFF A label that can be used when plotting data to distinguish this run from another. Use %varname to automatically add current variable value 
  String        extra_files;    // space separated list of extra files to check into the repository along with this project
  String        svn_repo;       // #NO_DIFF svn repository to use for file exchange with the cluster -- this should be the name of a svn_repo as listed in the Preferences / Options settings (when you Run a job you can pick from a dropdown list)
  ...
  String        queue;          // #NO_DIFF if specified, indicate a particular queue on the computing resource (optional) -- depends on cluster whether this is used
  String        run_time;       // #NO_DIFF how long will the jobs take to run -- syntax is number followed by unit indicator -- m=minutes, h=hours, d=days -- e.g., 30m, 12h, or 2d -- typically the job will be killed if it exceeds this amount of time, so be sure to not underestimate
...
public:
  ...
  // #IGNORE cancel any ongoing updating -- cleanup after exception
  ...
  virtual void  LoadDataNoRemove();
  // calls LoadData(false) which loads the data and keeps any existing data
