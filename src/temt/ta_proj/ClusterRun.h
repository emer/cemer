...
public:
  ...
  bool          load_only;        // set if we should only load data from local SVN working copy or update from remote repository
...
public:
  ...
  virtual bool  Update(bool load_only = false);
  // #BUTTON updates jobs_running and jobs_done tables based on latest results from the cluster (load_only true means just load tables from svn working copy, don't update from server) -- returns true if new data or status was available -- cluster only updates automatically if a job completes or a probe was sent from this project
