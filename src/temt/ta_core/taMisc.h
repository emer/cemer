...
public:
  int  labels; // #MIN_4 the label size preference includes widgets and dialog content
  int  navigator;  // #MIN_4 the font size used in the leftmost pane of the main window
...
class TA_API taFontNames {
  // #EDIT_INLINE font size preferences for various parts of the GUI
public:
  String  general; // font name for all ui elements other than the console
  String  console;  // font name for the css console
...
class TA_API taMisc {
  // #NO_TOKENS #INSTANCE ##STATIC_COMPLETION ##STATIC_MEMBERS global parameters and functions for the application
...
//NOTE: following not keeping tokens so cannot be viewed in any mode
...
#ifndef NO_TA_BASE
  static void DisplayList(taBase_PtrList& base_list, const String& title, const String_Array* addl_info = NULL, const String& info_title = "");
  ...
  static float  ScreenWindowScale();
  // #CAT_GUI get the screen scale multiplier for the current primary screen, from the user screen settings
  ...
  static String FullArgStringName(bool exclude_flags=true,
                     const String& exclude_names=NULLStr,
