...
void * thread_recv(void * data) {
  ...
  if (!recv_struct->connectedS[recv_struct->tag]) {
    recv_struct->serverconnfds[recv_struct->tag] =
      accept(recv_struct->serverfds[recv_struct->tag], 
...
ThreadedAllReduce::ThreadedAllReduce(MPI_Comm mpi_ctx, int nThreads) {
  ...
  if (taMisc::thread_defaults.node_prefix.nonempty()) {
    proc_name_tmp = (char *) calloc(MPI_MAX_PROCESSOR_NAME, sizeof(char));
    strncpy(proc_name_tmp, taMisc::thread_defaults.node_prefix.chars(),
  ...
  MPI_Allgather(proc_name, MPI_MAX_PROCESSOR_NAME, MPI_CHAR, dmem_proc_names,
                MPI_MAX_PROCESSOR_NAME, MPI_CHAR, mpi_ctx);
