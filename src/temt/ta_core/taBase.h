...
public:
  ...
  // #MENU #MENU_ON_ControlPanel #NO_MENU_PANEL #MENU_SEP_BEFORE #CAT_Display #NULL_OK_1 #NULL_TEXT_1_NewCtrlPanel select an object member to be added to a project control_panel - a panel that is a user chosen collection of members and methods from one or more objects (if ctrl_panel is NULL, a new one is created in .ctrl_panels).  returns false if member was already selected.
  ...
  // #MENU #NULL_OK_1  #NULL_TEXT_1_NewCtrlPanel  #CAT_Display select a function (method) for calling from a project control_panel - a panel that is a user chosen collection of members and methods from one or more objects (if ctrl_panel is NULL, a new one is created in .ctrl_panels). returns false if method was already selected.
  virtual bool          AddFunToControlPanelNm(const String& function_nm, ControlPanel* ctrl_panel, const String& desc = _nilString);
  // #CAT_Display select a method (by name) for use in a control_panel that is a collection of selected members and methods from different objects (if ctrl_panel is NULL, a new one is created in .crtl_panels)  returns false if method was already selected. desc is a custom description -- will show up as tooltip for user (default is info from member)
