...
public:
  ...
  // #CAT_Distance #MENU_BUTTON #MENU_ON_Distance using the normalized dot-product (cosine) distance metric, computes a within vs. between group (category) similarity metric, as: 1 - [(1 - within) / (1 - between)], where within and between are either the average or maximum cosines, used for avg_sim and max_sim respectively.  Also the average of the max of these values across categories is reported in the max_avg_sim and max_max_sim results.  This value is 1 when within items are identical and between are completely different (~0 cosine), and 0 when there is no diff between within vs. between (and negative if within is less similar than between).  Groups are defined as having the same name in name_col_nm -- source data must be sorted along this column.  If avg, max_sim_col_nm are provided, then the per-item similarity stats results are written to those column names in the src_data table (yes, that table is modified!)

  static void   DistMatrixGroupSimilarityMatrix(DataTable* dst_data, DataTable* cat_dst_data, DataTable* src_data,
                                   const String& data_col_nm, const String& name_col_nm);
  // #CAT_Distance #MENU_BUTTON #MENU_ON_Distance #NULL_OK_0 #NULL_TEXT_0_NewDataTable using the normalized dot-product (cosine) distance metric, computes overall category-wise average similarities across categories, which are stored in cat_dst_data table -- then sorts the overall element-wise distance table in dst_data by the most similar categories in order
