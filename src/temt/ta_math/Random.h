...
public:
  ...
  // generate a random variable according to current parameters -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  ...
  // #CAT_Int uniform random integer in the range between 0 and n, exclusive of n: [0,n) -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  static int    IntMinMax(int min, int max, int thr_no = 0)
  { return min + IntZeroN(max - min, thr_no); }
  // #CAT_Int uniform random integer in range between min and max, exclusive of max: [min,max) -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  static int    IntMeanRange(int mean, int range, int thr_no = 0)
  { return mean + (IntZeroN(2 * range + 1, thr_no) - range); }
  // #CAT_Int uniform random integer with given range on either side of the mean: [mean - range, mean + range] -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  static int    Discrete(taMatrix* distribution, int thr_no = 0);
  // #CAT_Int Discrete distribtion. Samples from a discrete distribution with probabilities given (automatically renormalizes the values) -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  static double ZeroOne(int thr_no = 0)
  { return MTRnd::GenRandRes53(thr_no); }
  // #CAT_Float uniform random number between zero and one (inclusive of 1 due to rounding!) -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  static double UniformMinMax(double min, double max, int thr_no = 0)
  { return min + (max - min) * ZeroOne(thr_no); }
  // #CAT_Float uniform random number between min and max values inclusive (Do not use for generating integers - will not include max!) -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  static double UniformMeanRange(double mean, double range, int thr_no = 0)
  { return mean + range * 2.0 * (ZeroOne(thr_no) - 0.5); }
  // #CAT_Float uniform random number with given range on either size of the mean: [mean - range, mean + range] -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)

  static double Binom(int n, double p, int thr_no = 0);
  // #CAT_Float binomial with n trials (par) each of probability p (var) -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  static double Poisson(double l, int thr_no = 0);
  // #CAT_Float poisson with parameter l (var) -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  static double Gamma(double var, double j, int thr_no = 0);
  // #CAT_Float gamma with given variance, number of exponential stages (par) -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  static double Gauss(double stdev, int thr_no = 0);
  // #CAT_Float gaussian (normal) random number with given standard deviation -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  static double Beta(double a, double b, int thr_no = 0);
  // #CAT_Float beta random number with two shape parameters a > 0 and b > 0 -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
  ...
  // #CAT_Bool boolean true/false with given probability -- specify thread number if calling from thread for thread-safe operation (1 <= thr_no < 100)
