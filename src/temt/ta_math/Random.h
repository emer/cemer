...
class TA_API Random : public taINBase {
  // #STEM_BASE #NO_UPDATE_AFTER ##INLINE ##INLINE_DUMP ##CAT_Math ##NO_TOKENS ##STATIC_COLLECTION Random Number Generation
INHERITED(taINBase)
public:
  enum Type {
  ...
  static int    Discrete(taMatrix* distribution, int thr_no = -1);
  ...
  static bool   BoolProb(double p, int thr_no = -1) { return (ZeroOne(thr_no) < p); }
