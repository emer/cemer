...
// single-arg float function on single float arg -- used for applying functions to vector
typedef float (*taMath_float_fun)(float);
...
//   // #CAT_Probability true gamma function (not gamma distribution): generalization of (n-1)! to real values
  ...
  static float beta_dev(float a, float b, int thr_no = -1);
  ...
  static float binom_dev(int n, float p, int thr_no = -1);
  ...
  static float poisson_dev(float l, int thr_no = -1);
  ...
  static float gamma_dev(float k, float lambda = 1.0, int thr_no = -1);
  ...
  static float gauss_dev(int thr_no = -1);
 ...
 // #CAT_Probability cumulative student's distribution df deg of free t test
  ...
  static float Ftest_q(float F, float df_num, float df_den);
  ...
  // #CAT_Statistics gets a histogram (counts) of number of values within each bin size in source vector -- min and maximum ranges to compute within are also optional args -- only used min != max -- optionally populate bin_vec with X axis bin values if non-null
  ...
  static int   vec_prob_choose(float_Matrix* vec, int thr_no = -1);
  ...
  static bool vec_jitter_gauss(float_Matrix* vec, float stdev, int thr_no = -1);
  ...
  static float vec_norm_mean(float_Matrix* vec, float trg_mean=0.0);
