...
public: // XxxGui versions provide feedback to the user
  ...
  // #MENU #MENU_SEP_BEFORE #MENU_ON_ControlPanel #NULL_OK_0  #NULL_TEXT_0_CtrlPanel #ITEM_FILTER_0_ControlPanelNoParamSetItemFilter #CAT_Display add the program control functions (Init, Run, Step, Stop) to a project control_panel - a panel that is a user chosen collection of members and methods from one or more objects (if ctrl_panel is NULL, a new one is created in .ctrl_panels). returns false if method was already selected. extra_label is prepended to item names, and if sub_gp_nm is specified, items will be put in this sub-group (new one will be made if it does not yet exist)
