...
class TA_API ProgVar: public taNBase {
  // ##INSTANCE ##EDIT_INLINE #STEM_BASE ##SCOPE_Program ##CAT_Program ##DIFF_STRING a program variable, accessible from the outer system, and inside the script in .vars and args
INHERITED(taNBase)
public:
  enum VarType {
  ...
  enum VarFlags { // #BITS flags for modifying program variables
    ...
    PGRM_ARG            = 0x0400, // #HIDDEN this is a program argument variable
  };
