...
void taiWidgetActions::ActionAdded(taiAction* it) {
  actionsRep()->addAction(it);
}

void taiWidgetActions::ActionRemoving(taiAction* it) {
...
void taiWidgetActions::AddAction(taiAction* act) {
...
taiAction* taiWidgetActions::AddItem(
  const String& val,
  SelType st,
...
taiAction* taiWidgetActions::AddItemWithNumericAccel(
  const String& val,
  taiAction::CallbackType ct,
  const QObject *receiver,
  const char* member,
...
taiMenu* taiWidgetActions::AddSubMenu(const String& val, TypeDef* typ_) {
...
void taiWidgetActions::child_triggered_toggled(taiAction* act) {
...
taiAction* taiWidgetActions::curSel() const {
  if (par_menu != NULL)
...
taiAction* taiWidgetActions::FindActionByData(const Variant& usr) {
  for (int i = 0; i < items.size; ++i) {
...
taiMenu* taiWidgetActions::FindSubMenu(const String& nm) {
  for (int i = 0; i < items.size; ++i) {
    taiAction* itm = items.FastEl(i);
...
void taiWidgetActions::setCurSel(taiAction* value) {
