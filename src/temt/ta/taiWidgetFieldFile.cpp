...
#include "taiWidgetFieldFile.h"

#include <iLineEdit>
#include <taFiler>


taiWidgetFieldFile::taiWidgetFieldFile(TypeDef* typ_, IWidgetHost* host_, taiWidget* par,
                                       QWidget* gui_parent_, int flags_, FileActionType fact,
                                       const String& fext, const String& ftyp, int fcmprs)
  : taiWidgetText(typ_, host_, par, gui_parent_, flags_, true,
...
void taiWidgetFieldFile::btnEdit_clicked(bool) {
  lookupKeyPressed();           // all the code is there
}

void taiWidgetFieldFile::lookupKeyPressed() {
  taFiler* flr = NULL;
  if(base_obj) {
