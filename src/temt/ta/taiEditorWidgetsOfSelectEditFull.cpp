...
#include "taiEditorWidgetsOfSelectEditFull.h"
#include <SelectEdit>
#include <iLabel>
#include <iFlowLayout>
#include <taiMember>
...
taiEditorWidgetsOfSelectEditFull::taiEditorWidgetsOfSelectEditFull(void* base, TypeDef* td,
...
taiEditorWidgetsOfSelectEditFull::~taiEditorWidgetsOfSelectEditFull() {
}

void taiEditorWidgetsOfSelectEditFull::Initialize()
{
}

void taiEditorWidgetsOfSelectEditFull::ClearBody_impl() {
...
void taiEditorWidgetsOfSelectEditFull::Constr_Data_Labels() {
...
void taiEditorWidgetsOfSelectEditFull::DoRemoveSelEdit() {
...
void taiEditorWidgetsOfSelectEditFull::FillLabelContextMenu_SelEdit(QMenu* menu,
  int& last_id)
{
...
void taiEditorWidgetsOfSelectEditFull::GetImage_Membs_def() {
  int itm_idx = 0;
  for (int j = 0; j < membs.size; ++j) {
    ...
    for (int i = 0; i < ms->data_el.size; ++i) {
      ...
      if ((item == NULL) || (item->base == NULL) || (md == NULL) || (mb_dat == NULL)) {
        taMisc::DebugInfo("taiEditorWidgetsOfSelectEditFull::GetImage_impl(): unexpected md or mb_dat=NULL at i ", String(i));
      }
      else {
...
void taiEditorWidgetsOfSelectEditFull::GetValue_Membs_def() {
  int itm_idx = 0;
  for (int j = 0; j < membs.size; ++j) {
    MembSet* ms = membs.FastEl(j);
    for (int i = 0; i < ms->data_el.size; ++i) {
