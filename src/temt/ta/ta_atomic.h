...
class TA_API QAtomicInt { // this copies the barest API of QBasicAtomicInt/QAtomicInt, which is a Plain Old Data type
public:
...
# ifdef TA_USE_QT
  inline bool QAtomicInt::ref();
    {return q_atomic_increment(&_q_value) != 0;}
  
  inline bool QAtomicInt::deref()
    {return q_atomic_decrement(&_q_value) != 0;}
  
  inline bool QAtomicInt::testAndSetOrdered(int expectedValue, int newValue)
    {return q_atomic_test_and_set_int(&_q_value, expectedValue, newValue) != 0;}
  
  inline int QAtomicInt::fetchAndStoreOrdered(int newValue)
    {return q_atomic_set_int(&_q_value, newValue);}
  
  inline int QAtomicInt::fetchAndAddOrdered(int valueToAdd)
