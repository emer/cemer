...
// File and Path routines
  ...
  static String 	noFinalSep(const String& in); // return string that has no final separator
  ...
  static String 	getDocPath(); // the user's Documents path, (or home, if none defined)
  static String 	getHomePath(); // the user's home folder
  static String 	getAppDataPath(const String& appname); // root for preference data, typically hidden from user
  static String 	getAppDocPath(const String& appname); // root for user-visible application files
  ...
  static bool		mv(const String& fm, const String& to); // move a file or folder, true if ok (fm should exist, 'to' should not, no files should be open, and may need to be on same partition)
