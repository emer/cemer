...
#include "taiEditorWidgetsOfListCompact.h"
#include <taList_impl>
#include <taBase_List>
#include <taiType>
#include <iEditGrid>
...
taiEditorWidgetsOfListCompact::taiEditorWidgetsOfListCompact(void* base, TypeDef* typ_, bool read_only_,
...
taiEditorWidgetsOfListCompact::~taiEditorWidgetsOfListCompact() {
  lst_data_el.Reset();
}

void taiEditorWidgetsOfListCompact::ClearMultiBody_impl() {
  lst_data_el.Reset();
  inherited::ClearMultiBody_impl();
}


void taiEditorWidgetsOfListCompact::Constr_Strings() {
...
bool taiEditorWidgetsOfListCompact::ShowMember(MemberDef* md) const {
  return md->ShowMember(show());
}

void taiEditorWidgetsOfListCompact::Constr_ElData() {
...
void taiEditorWidgetsOfListCompact::Constr_ListData() {
...
void taiEditorWidgetsOfListCompact::GetValue_Membs() {
  bool rebuild = false;
...
void taiEditorWidgetsOfListCompact::GetImage_Membs() {
  bool rebuild = false;
  if (lst_data_el.size != cur_lst->size) rebuild = true;
  if (!rebuild) {		// check that same elements are present!
    for (int lf = 0;  lf < lst_data_el.size;  ++lf) {
...
/* TODO
int taiEditorWidgetsOfListCompact::Edit() {
