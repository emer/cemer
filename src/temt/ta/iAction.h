...
class TA_API iAction: public QAction {
  ...
  friend class iAction_List;
public:
  enum CallbackType {
    none,               // callback parameters ignored
    action,             // corresponds to IV "action" type of callback -- slot for signal is parameterless
    men_act,            // corresponds to IV "men_act" (menu action) type of callback -- slot for signal takes iAction* as a sender
  ...
  bool                  canSelect(); // true if item can be taiWidgetMenu::curSel value
  virtual bool          isSubMenu()     { return false; }
  bool                  isGrouped(); // indicates if item in a group, ie is a radio group action

  iAction(int sel_type_, const String& label_); // used by taiWidgetMenu etc.
  iAction(const QString& label_, const QKeySequence& accel, const char* name); // used by viewer/browser
  iAction(const QString& label_, QObject* receiver, const char* member, const QKeySequence& accel);
    // used by viewer/browser
  iAction(const Variant& usr_data_, const QString& label_, const QKeySequence& accel, const char* name = NULL); // used by viewer/browser
  virtual ~iAction();

  void connect(CallbackType ct_, const QObject *receiver, const char* member); // connect callback to given
  void connect(const iMenuAction* mact); // convenience function
...
signals:
  ...
  void MenuAction(iAction* sender);   // for a menu-action call-back (returns item selected)
...
protected:
  int                   nref; // ref'ed when put onto a iAction_List, deleted when ref=0
...
private:
  iAction(const iAction&); // no copying
  void operator=(const iAction&);
};
