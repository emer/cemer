...
#include "taiEditorWidgetsOfList.h"
#include <taList_impl>
#include <taBase_List>
#include <MemberDef>
#include <iEditGrid>
...
taiEditorWidgetsOfList::taiEditorWidgetsOfList(void* base, TypeDef* typ_, bool read_only_,
...
taiEditorWidgetsOfList::~taiEditorWidgetsOfList() {
  lst_data_el.Reset();
  lst_membs.Reset();
}

void taiEditorWidgetsOfList::ClearMultiBody_impl() {
  lst_data_el.Reset();
  lst_membs.Reset();
  num_lst_fields = 0;
...
void taiEditorWidgetsOfList::Constr_Strings() {
...
void taiEditorWidgetsOfList::Constr_ElData() {
...
void taiEditorWidgetsOfList::Constr_ListData() {
...
void taiEditorWidgetsOfList::Constr_ListLabels() {
...
void taiEditorWidgetsOfList::GetValue_Membs() {
  bool rebuild = false;
...
void taiEditorWidgetsOfList::GetImage_Membs() {
  bool rebuild = false;
  if (lst_data_el.size != cur_lst->size) rebuild = true;
  if (!rebuild) {		// check that same elements are present!
    for (int lf = 0;  lf < lst_data_el.size;  ++lf) {
...
/* TODO
int taiEditorWidgetsOfList::Edit() {
