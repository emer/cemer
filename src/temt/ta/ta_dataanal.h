...
public:
  ...
  static bool	CrossDistMatrix(float_Matrix* dist_mat,
				...
				taMath::DistMetric metric, bool norm=false, float tol=0.0f);
  // #CAT_Distance compute cross distance matrix between two different matrix data columns in src_data_a and src_data_b datatables. if col_nm's are both blank, all real-valued matrix columns are used, summing across (only cols at same col index that match exactly will be used). dist_mat returns a symmetric matrix with cells as the distance between each row in table a versus each row of table b.  distance data is converted to float regardless of source type (float or double)
