...
#include "taiWidgetTypeItemMenu.h"
#include <taiWidgetMenu>
#include <MemberDef>

#include <taMisc>

taiWidgetTypeItemMenu::taiWidgetTypeItemMenu(taiWidgetActions::RepType rt, int ft,
  ...
  : taiWidget(typ_, host_, par, gui_parent_, flags_)
{
  ...
  memb_md = memb_md_;
  menubase = NULL;
  ta_actions = taiWidgetActions::New(rt, taiWidgetMenu::radio_update, ft, NULL, host_, this, gui_parent_);
}

taiWidgetTypeItemMenu::~taiWidgetTypeItemMenu() {
  if (ta_actions != NULL) {
    delete ta_actions;
    ta_actions = NULL;
  }
...
void taiWidgetTypeItemMenu::GetImage(const void* base, bool get_menu, void* cur_sel){
...
QWidget* taiWidgetTypeItemMenu::GetRep() {
  if (ta_actions) return ta_actions->GetRep();
  else return NULL;
}

void taiWidgetTypeItemMenu::GetTarget() {
