...
#include "taiEditorWidgetsOfProgramCtrl.h"
#include <Program>
...
taiEditorWidgetsOfProgramCtrl::~taiEditorWidgetsOfProgramCtrl() {
}

bool taiEditorWidgetsOfProgramCtrl::ShowMember(MemberDef* md) const {
  return false;
}

void taiEditorWidgetsOfProgramCtrl::Cancel_impl() {
  refs.setOwner(NULL);
...
void taiEditorWidgetsOfProgramCtrl::Enum_Members() {
...
void taiEditorWidgetsOfProgramCtrl::Constr_Data_Labels() {
  dat_cnt = 0;
...
void taiEditorWidgetsOfProgramCtrl::SigDestroying_Ref(taBase_RefList* ref, taBase* base) {
  // we need to rebuild...
  if (ref == &refs)
    ReShow_Async();
...
void taiEditorWidgetsOfProgramCtrl::SigEmit_Ref(taBase_RefList* ref, taBase* base,
...
MemberDef* taiEditorWidgetsOfProgramCtrl::GetMemberPropsForSelect(int sel_idx, taBase** base,
...
void taiEditorWidgetsOfProgramCtrl::GetValue_Membs_def() {
  Program* prog = this->prog(); //cache
  if (!prog) return;
...
//   }
  ...
  for (int j = MS_ARGS; j <= MS_VARS; ++j) {
    ...
    for (int i = 0; i < pvl->size; ++i) {
      ...
      if (!md || !mb_dat) {
        taMisc::DebugInfo("taiEditorWidgetsOfProgramCtrl:GetValue_impl: ran out of controls!");
        break;
      }
      if(pv->var_type == ProgVar::T_HardEnum) {
...
void taiEditorWidgetsOfProgramCtrl::GetImage_Membs()
{
  Program* prog = this->prog(); //cache
  if (!prog) return;
  ...
  for (int j = MS_ARGS; j <= MS_VARS; ++j) {
    ...
    for (int i = 0; i < pvl->size; ++i) {
      ...
      taiData* mb_dat = data_el(j).SafeEl(cnt++);
      if (!md || !mb_dat) {
        taMisc::DebugInfo("taiEditorWidgetsOfProgramCtrl:GetImage_impl: ran out of controls!");
        break;
      }
