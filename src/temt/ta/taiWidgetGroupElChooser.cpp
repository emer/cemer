...
#include "taiWidgetGroupElChooser.h"
...
taiWidgetGroupElChooser::taiWidgetGroupElChooser(TypeDef* typ_, IWidgetHost* host,
...
const String taiWidgetGroupElChooser::titleText() {
  String chs_title = "Choose " + itemTag();
  if(grp) chs_title += " from group: " + grp->GetDisplayName();
  return chs_title;
}

void taiWidgetGroupElChooser::BuildChooser(iDialogItemChooser* ic, int view) {
  //assume only called if needed
  
  if (!grp) {
    taMisc::Error("taiWidgetGroupElChooserBase::BuildChooser: group needed");
    return;
  }
  switch (view) {
  case 0: 
...
int taiWidgetGroupElChooser::BuildChooser_1(iDialogItemChooser* ic, taGroup_impl* top_grp, 
...
void taiWidgetGroupElChooser::GetImage(taGroup_impl* base_grp, taBase* it) {
  grp = base_grp;
