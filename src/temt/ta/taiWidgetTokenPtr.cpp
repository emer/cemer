...
taiWidgetTokenPtr::taiWidgetTokenPtr(TypeDef* typ_, IWidgetHost* host,
...
void taiWidgetTokenPtr::btnHelp_clicked() {
  taBase* tok = token();
  if (!tok) return;
...
void taiWidgetTokenPtr::EditDialog() {
...
void taiWidgetTokenPtr::EditPanel() {
  taBase* cur_base = GetValue();
  if (!cur_base) return;
...
bool taiWidgetTokenPtr::countTokensToN(int& cnt, TypeDef* td, int n, void*& last_itm) {
...
bool taiWidgetTokenPtr::hasNoItems() {
...
bool taiWidgetTokenPtr::hasOnlyOneItem() {
  if(!targ_typ) return false;          // shouldn't happen
  int cnt = 0;
  void* last_itm = NULL;
...
void taiWidgetTokenPtr::BuildChooser(iDialogItemChooser* ic, int view) {
  if(cust_chooser)
    cust_chooser(scope_ref, this);
  inherited::BuildChooser(ic, view);
  if (!targ_typ) {
    taMisc::Error("taiWidgetTokenPtr::BuildChooser: targ_type needed");
    return;
  }
  switch (view) {
  case 0:
...
int taiWidgetTokenPtr::BuildChooser_0(iDialogItemChooser* ic, TypeDef* td,
...
int taiWidgetTokenPtr::columnCount(int view) const {
  switch (view) {
  case 0: return 5;
...
void taiWidgetTokenPtr::GetImage(void* cur_sel_, TypeDef* targ_typ_)
...
void taiWidgetTokenPtr::GetImageScoped(taBase* ths, TypeDef* targ_typ_,
...
const String taiWidgetTokenPtr::headerText(int index, int view) const {
  switch (view) {
  case 0: switch (index) {
    case 0: return "Name";
    case 1: return "Type";
...
const String taiWidgetTokenPtr::labelNameNonNull() const {
  return token()->GetDisplayName();
}

bool taiWidgetTokenPtr::ShowToken(taBase* obj) const {
  return ShowItemFilter(scope_ref, obj, obj->GetName());
}

const String taiWidgetTokenPtr::viewText(int index) const {
  switch (index) {
