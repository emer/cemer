...
taiWidgetItemPtr::taiWidgetItemPtr(TypeDef* typ_,
...
taiWidgetItemPtr::~taiWidgetItemPtr() {
  delete cats;
  cats = NULL;
}

void taiWidgetItemPtr::BuildChooser(iDialogItemChooser* ic, int view) {
  if(new1_par) {
...
void taiWidgetItemPtr::BuildCategories() {
  if (!isValid()) return;
  BuildCategories_impl();
}

int taiWidgetItemPtr::catCount() const {
...
const String taiWidgetItemPtr::catText(int index) const {
  return (cats) ? cats->SafeEl(index) : _nilString;
}

void taiWidgetItemPtr::GetImage(void* cur_sel, TypeDef* targ_typ_) {
  targ_typ = targ_typ_;
  UpdateImage(cur_sel);
}

const String taiWidgetItemPtr::labelText() {
  String nm;
...
const String taiWidgetItemPtr::titleText() {
  String chs_title = "Choose " + itemTag();
  if(targ_typ) chs_title += " from: " + targ_typ->name;
  return chs_title;
}

bool taiWidgetItemPtr::OpenChooser() {
  bool rval = false;
...
bool taiWidgetItemPtr::ShowItemFilter(void* base, void* item, const String& itnm) const {
...
void taiWidgetItemPtr::UpdateImage(void* cur_sel) {
