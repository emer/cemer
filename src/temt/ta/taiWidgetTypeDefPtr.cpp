...
taiWidgetTypeDefPtr::taiWidgetTypeDefPtr(TypeDef* typ_, IWidgetHost* host,
...
taiWidgetTypeDefPtr::TypeCat taiWidgetTypeDefPtr::AddType_Class(TypeDef* typ_) {
...
void taiWidgetTypeDefPtr::btnHelp_clicked() {
  TypeDef* typ = td();
  if (!typ) return;
  iHelpBrowser::StatLoadType(typ);
}

void taiWidgetTypeDefPtr::BuildCategories_impl() {
  if (cats) cats->Reset();
...
void taiWidgetTypeDefPtr::BuildCategoriesR_impl(TypeDef* top_typ) {
...
bool taiWidgetTypeDefPtr::hasNoItems() {
  // always false for this -- targ_typ is snould not be null!
  return false;
}

bool taiWidgetTypeDefPtr::hasOnlyOneItem() {
...
void taiWidgetTypeDefPtr::BuildChooser(iDialogItemChooser* ic, int view) {
  inherited::BuildChooser(ic, view);
  if (!targ_typ) {
    taMisc::Error("taiWidgetTypeDefPtr::BuildChooser: targ_type needed");
    return;
  }
  switch (view) {
  case 0:
...
int taiWidgetTypeDefPtr::BuildChooser_0(iDialogItemChooser* ic, TypeDef* top_typ,
...
int taiWidgetTypeDefPtr::columnCount(int view) const {
  switch (view) {
  case 0: return 3;
...
int taiWidgetTypeDefPtr::CountChildren(TypeDef* td) {
  int rval = 0;
  TypeDef* chld;
...
const String taiWidgetTypeDefPtr::headerText(int index, int view) const {
  switch (view) {
  case 0: switch (index) {
    case 0: return "Type";
...
const String taiWidgetTypeDefPtr::labelNameNonNull() const {
  return td()->name;
}

const String taiWidgetTypeDefPtr::viewText(int index) const {
  switch (index) {
