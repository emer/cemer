...
#include "taiWidgetMethodDefPtr.h"
...
taiWidgetMethodDefPtr::taiWidgetMethodDefPtr(TypeDef* typ_, IWidgetHost* host,
...
void taiWidgetMethodDefPtr::btnHelp_clicked() {
  MethodDef* mth = md();
  if (!mth) return;
  iHelpBrowser::StatLoadMethod(mth);
}

void taiWidgetMethodDefPtr::BuildCategories_impl() {
  if (cats) cats->Reset();
...
void taiWidgetMethodDefPtr::BuildChooser(iDialogItemChooser* ic, int view) {
  inherited::BuildChooser(ic, view);
  if (!targ_typ) {
    taMisc::Error("taiWidgetMethodDefPtr::BuildChooser: targ_type needed");
    return;
  }
  switch (view) {
  case 0:
...
void taiWidgetMethodDefPtr::BuildChooser_0(iDialogItemChooser* ic) {
  MethodSpace* mbs = &targ_typ->methods;
  String cat;
  for (int i = 0; i < mbs->size; ++i) {
    MethodDef* mth = mbs->FastEl(i);
...
int taiWidgetMethodDefPtr::BuildChooser_1(iDialogItemChooser* ic, TypeDef* top_typ,
...
void taiWidgetMethodDefPtr::BuildChooser_2(iDialogItemChooser* ic) {
  MethodSpace* mbs = &targ_typ->methods;
  String cat;
  for (int i = 0; i < mbs->size; ++i) {
    MethodDef* mth = mbs->FastEl(i);
...
int taiWidgetMethodDefPtr::columnCount(int view) const {
  switch (view) {
  case 2:
  case 0: return 2;
  case 1: return 3;
...
const String taiWidgetMethodDefPtr::headerText(int index, int view) const {
  switch (view) {
  case 2:
  case 0: {
    switch (index) {
...
const String taiWidgetMethodDefPtr::labelNameNonNull() const {
  return md()->name;
}

bool taiWidgetMethodDefPtr::ShowMethod(MethodDef* mth) {
  return (ShowItemFilter(NULL, mth, mth->name) &&  mth->ShowMethod());
}

const String taiWidgetMethodDefPtr::viewText(int index) const {
  switch (index) {
