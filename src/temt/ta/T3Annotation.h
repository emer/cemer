...
public:
  ...
  taVector3f    start;          // #CONDSHOW_ON_type:LINE,ARROW,DOUBLEARROW starting coordinates for lines or arrows
  taVector3f    end;            // #CONDSHOW_ON_type:LINE,ARROW,DOUBLEARROW ending coordinates for lines or arrows
  taVector3f    bot_left;       // #CONDSHOW_ON_type:ELLIPSE,RECTANGLE bottom left coordinates for rectangles or ellipses (defines the bounding box for the ellipse)
  taVector3f    top_right;      // #CONDSHOW_ON_type:ELLIPSE,RECTANGLE top right coordinates for rectangles or ellipses (defines the bounding box for the ellipse)
  ...
  taVector3f    pos;            // 3d position of object (can be moved through a dragger too)
  ...
  virtual void  SetLine(float st_x, float st_y, float st_z,
                        float ed_x, float ed_y, float ed_z,
                        float ln_width = 1.0, const String& color = "black");
  // set to a line with given coordinates
  virtual void  SetArrow(float st_x, float st_y, float st_z,
                         float ed_x, float ed_y, float ed_z,
                         float ln_width = 1.0, const String& color = "black",
                         float arrow_sz = 0.02);
  // add an arrow annotation -- coordinates are in the normalized coordinates of the view (typically 1x1x1) 
  virtual void  SetDoubleArrow(float st_x, float st_y, float st_z,
                               float ed_x, float ed_y, float ed_z,
                               float ln_width = 1.0, const String& color = "black",
                               float arrow_sz = 0.02);
  // add a double-arrow annotation -- coordinates are in the normalized coordinates of the view (typically 1x1x1) 
  virtual void  SetRectangle(float bot_left_x, float bot_left_y, float bot_left_z,
  ...
  virtual void  SetEllipse(float bot_left_x, float bot_left_y, float bot_left_z,
                           float top_right_x, float top_right_y, float top_right_z,
                           float ln_width = 1.0, const String& color = "black");
