...
static int nItems2 = 120*1024*1024;
...
int main(int argc, char ** argv) {
    ...
    myreduce = new AllReduce(MPI_COMM_WORLD, nThreads);
    ...
    if (0) {
        ...
        printf("We did the threaded reduce in %f seconds at a rate of %f Gbit/s\n", (timer1e - timer1s), ((nItems2*sizeof(float)*8.0*reps)/(timer1e - timer1s) / 1024.0 / 1024.0 / 1024.0));
        
        broken = 0;
        for (int i = 0; i < nItems2; i++) {
            if (data2_recv[i] != data2_recv_thr[i])
                broken++;
        }
        printf("\n %i values were different!\n", broken);
        
        printf("Threads were supposed to have been created by now\n");
    ...
    if (do_threaded) {
        ...
        for (int j = 0; j < nThreads; j++) {
            thread_data[j] = j;
            pthread_create(&(thread_ids[j]), NULL,  &thread_routine2, (void *) &thread_data[j]);
        }
        ...
        printf("P%i: We did the threaded self reduce in %f seconds at a rate of %f Gbit/s\n", dmem_proc, (timer1e - timer1s), ((nItems2*sizeof(float)*8.0*reps)/(timer1e - timer1s) / 1024.0 / 1024.0 / 1024.0));
    }
