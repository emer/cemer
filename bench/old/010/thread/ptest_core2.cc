...
#else // unix
inline int AtomicAdd(int* operand, int incr)
{
...
void Send_Netin_0(Unit* su) {
  ...
  Unit** uns = su->units; // unit pointer
  for(int i=0; i<n_units; i++)
...
void NetInTask_N::run() {
  ...
  int my_u = AtomicAdd(&g_u, core_nprocs);
  while (my_u < n_units_flat) {
    Unit* un = layers_flat[my_u]; //note: accessed flat
    if (un->DoDelta()) {
      Send_Netin_N(un);
      // AtomicAdd(&n_tot, 1);
      //AtomicAdd(&t_tot, 1); // because of helping hand clobbers
    }
    my_u = AtomicAdd(&g_u, core_nprocs);
  }
}
